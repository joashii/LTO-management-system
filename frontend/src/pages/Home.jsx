...
export default function Home({ onEnter }) {
  // Check sessionStorage on load to see if they've already played the intro
  const [stage, setStage] = useState(() => {
    return sessionStorage.getItem('hasSeenIntro') ? 'content' : 'prompt';
  }); 
    ...
    setTimeout(() => {
      if (videoRef.current) {
        videoRef.current.play().catch(err => {
          console.log("Playback prevented, defaulting to muted:", err);
          if (videoRef.current) {
  ...
  // Mark the intro as "seen" in the browser's session storage
  const finishIntro = () => {
  ...
  return (
    <div className={`home-wrapper ${isExiting ? 'exiting' : ''}`}>
      
      {/* BROWSER AUDIO UNBLOCK PROMPT */}
      {stage === 'prompt' && (
        <div className="intro-prompt-overlay">
          <div className="prompt-card">
            <img src={logo} alt="LTO Logo" className="prompt-logo" />
            <h2>LTO Management System</h2>
            <p>Experience the cinematic system introduction</p>
            <button className="prompt-enter-btn" onClick={startIntro}>
              Enter System
            </button>
          </div>
        </div>
      ...
      {/* VIDEO LAYER */}
      <div className={`video-intro-container ${stage !== 'video' ? 'video-fade-out' : ''}`}>
        {stage === 'video' && (
          <>
            <video
              ref={videoRef}
              ...
              src={introVideo}
              playsInline
              onEnded={finishIntro} /* <-- Uses finishIntro */
            />
            <button className="skip-video-btn" onClick={finishIntro}> {/* <-- Uses finishIntro */}
              Skip Intro
            </button>
          </>
        )}
      ...
      {/* MAIN CONTENT */}
      <div className={`home ${stage === 'content' ? 'fade-in' : 'hidden'}`}>
        <nav className="home-nav">
          ...
          <div className="home-nav-right">
            ...
            <button className="nav-btn-filled" onClick={handleLeavePage}>Admin Login</button>
          </div>
        </nav>
        ...
        <section className="home-hero">
          ...
          <div className="hero-content">
            ...
            <button className="hero-cta" onClick={handleLeavePage}>
